...
struct DeepSampler : transvoxel::IDeepSDFSampler {
	...
	float get_single(Vector3i position_in_voxels, uint32_t lod_index) const override {
		...
		std::shared_ptr<VoxelBufferInternal> voxels;
		{
			...
			const Vector3i lod_bpos = lod_pos >> lod.map.get_chunk_size_pow2();
			const VoxelChunkData *chunk = lod.map.get_chunk(lod_bpos);
			...
			if (chunk != nullptr && chunk->has_voxels()) {
				voxels = chunk->get_voxels_shared();
				bsm = lod.map.get_chunk_size_mask();
			}
		}
		if (voxels != nullptr) {
